...
%%
% This function calculates an array of predicted signal data using the
% following relation:
%%
% $$ Mxy_{measurement} = Mz_{previous~measurement} \times cos(alpha \times 
% \theta)\times exp\left(-\frac{t}{T1}\right) \times 
% sin(alpha \times \theta) $$
%%
% where:
%% 
% $$ Mz_{previous~measurement} =
% \frac{Mxy_{previous~measurement}}{sin(alpha \times 
% \theta_{previous~measurement})} $$
%% 
...
function f = T1DecayFunction(xn,xdataf)
    ...
    f(1) = xn(1)*cosd(xn(2).*xdataf(1,2))*exp(-1*(xdataf(1,1)/xn(3)))*sind(xn(2).*xdataf(1,2));
    
    %% Calculating the Remaining Values of the Output Array
    
    for el = 2:length(f)
        f(el) = (f(el-1)/sind(xn(2).*xdataf(el-1,2)))*cosd(xn(2).*xdataf(el,2))*exp(-1*((xdataf(el,1)-xdataf(el-1,1)))/xn(3))*sind(xn(2).*xdataf(el,2));
    end
...
%% 
% f = T1DecayFunction([6949 1.3592 44.4454],[1 5; 2 5; 3 10; 4 10; 5 15; 6
% 15; 7 5])
